...
const Header = ({ header }) => {
    return (
        <View className={`flex flex-row items-center justify-between mx-2 mb-4 ${header === 'Spotlight' ? 'absolute top-0 z-50 w-full mx-0 mb-0 px-2' : ''}`}>
            <View className="flex flex-row gap-2 items-center">
                ...
                <Pressable className="bg-black/5 rounded-full w-[40px] h-[40px] flex justify-center items-center relative overflow-hidden">
                    <Ionicons name="search" size={20} color={header === 'Spotlight' ? 'white' : '#555c57'} />
                </Pressable>
            </View>
            <Text className={`text-[1.35rem] font-semibold tracking-wider ${header === 'Spotlight' ? 'text-white mr-[40px]' : ''}`}>{header}</Text>
            <View className="flex flex-row gap-2 items-center">
                {header !== 'Spotlight' && (
